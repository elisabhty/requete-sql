...
- **73 leçons** guidées (situation → anatomie → exercice → QCM)
...
`node tests/curriculum-production.test.mjs` vérifie les 73 cours, les solutions dans le moteur SQLite embarqué, les résultats annoncés des variantes et les validations des exercices (tri, alias, contraintes, transactions, triggers et droits). Les exemples explicitement non exécutables sont exclus.
