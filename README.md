...
- **74 leçons** guidées (situation → anatomie → exercice → QCM)
...
- **Cartes** : collection de cartes débloquées
- **Notes** : stylo ✎ disponible partout
...
`node tests/curriculum-production.test.mjs` vérifie les 74 cours, les solutions dans le moteur SQLite embarqué, les résultats annoncés des variantes et les validations des exercices (tri, alias, contraintes, transactions, triggers et droits). Les exemples explicitement non exécutables sont exclus.
