...
assert(guides.includes("k:'decimal'") && guides.includes("DECIMAL(10,2)") && !guides.includes('Que signifie DECIMAL') && !guides.includes('cf-dec-marks'), 'texte vers décimal');
...
assert(!guides.includes('sqliteSql') && !guides.includes("k:'precision'") && !guides.includes("k:'invalid'") && !guides.includes('live:false') && html.includes('function sqliteRunnableSql') && html.includes("printf('%.${scale}f'") && !guides.includes("printf('%.2f'"), 'CAST partout, tous les onglets exécutables');
...
assert(studio.includes('L’essentiel sur les conversions de types') && !studio.includes('CAST d’abord, puis ton SGBD') && studio.includes('Convertir change le type de la valeur. Formater change sa présentation.') && studio.includes('class="cf-attn"') && studio.includes('<b>Attention</b> Une conversion peut échouer') && studio.includes('<li>texte → entier</li>') && studio.includes('<li>texte → décimal</li>') && studio.includes('<li>nombre → texte</li>') && studio.includes('<li>texte → date</li>') && studio.includes('<li>date + heure → date</li>') && studio.includes('<li>entier → décimal</li>') && !studio.includes('texte → nombre') && !studio.includes('nombre → autre type numérique'), 'récapitulatif final recentré sur l’essentiel');
...
assert(serviceWorker.includes('requete-2026-09-25-join3-probleme-v1071'), 'cache de production renouvelé');
